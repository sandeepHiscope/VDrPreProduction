...
export default defineConfig({
  plugins: [
    ...
    tailwindcss(),
    VitePWA({
      registerType: "prompt", // User is prompted to register the service worker
      injectRegister: "auto", // Automatically inject service worker registration script
      strategies: "injectManifest", // Use the custom service worker file
      filename: "sw.js", // Name of your custom service worker file
      srcDir: "src", // The directory where your custom service worker file is located
      includeAssets: [
        "favicon.svg",
        "robots.txt",
        "offline.html",
      ...
      injectManifest: {
        ...
        swDest: "sw.js", // Output name of the service worker file in the dist folder
      },
      workbox: {
        swDest: "sw.js", // Ensure the service worker is generated at the root
        globDirectory: "dist", // Output directory
        globPatterns: ["**/*.{js,css,html,ico,png,svg}"], // Files to cache
        navigateFallback: "/index.html", // Fallback for single-page apps
        navigateFallbackDenylist: [/^\/api\//], // Don't fallback API routes
      ...
      devOptions: {
        ...
        type: "module", // Use ES module type for development
        navigateFallback: "index.html", // Fallback for development
      },
    }),
  ],
});
